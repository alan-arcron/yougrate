...
export async function up(knex: Knex): Promise<void> {
  // Drop the existing check constraint and add a new one with additional values
...
export async function down(knex: Knex): Promise<void> {
  await knex.raw(`ALTER TABLE "migrations" DROP CONSTRAINT IF EXISTS "migrations_status_check"`);
  await knex.raw(`ALTER TABLE "migrations" ADD CONSTRAINT "migrations_status_check" CHECK ("status" IN ('pending', 'analyzing', 'estimated', 'confirmed', 'running', 'completed', 'failed'))`);
}
